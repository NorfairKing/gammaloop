...
impl GenerationData {
    ...
    fn insert_thermal_numerator(&mut self, thermal_numerator_id: ThermalNumeratorID) {
        self.thermal_numerator_id = Some(thermal_numerator_id);
    }
}
...
pub(crate) fn generate_supergraph_cff(graph: &Graph) -> Result<CFFExpression<OrientationID>> {
    ...
    let mut orientations = get_orientations_from_subgraph(
        &graph.underlying,
    ...
    let mut result = generate_cff_from_orientations(
        orientations,
        &mut surface_cache,
        &[],
        &None,
